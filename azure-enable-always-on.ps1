...
Write-Host "[OK] Found $($webAppArray.Count) Web Apps" -ForegroundColor Green
...
foreach ($appName in $webAppArray) {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "  [OK] Always On enabled" -ForegroundColor Green
        $successCount++
    } else {
        Write-Host "  [FAIL] Failed to enable Always On" -ForegroundColor Red
        $failCount++
    }
}
...
Write-Host "[OK] Enabled: $successCount" -ForegroundColor Green
Write-Host "[SKIP] Already enabled: $skipCount" -ForegroundColor Gray
if ($failCount -gt 0) {
    Write-Host "[FAIL] Failed: $failCount" -ForegroundColor Red
}

if ($failCount -eq 0) {
    Write-Host "`n[SUCCESS] ALL WEB APPS CONFIGURED SUCCESSFULLY" -ForegroundColor Green
} else {
    Write-Host "`n[WARNING] SOME WEB APPS FAILED - CHECK ERRORS ABOVE" -ForegroundColor Yellow
}
...
Write-Host "  - Eliminates cold start delays (74s to 133s observed)" -ForegroundColor White
...
Write-Host "  - Your apps are on B1 tier [OK]" -ForegroundColor Green
...
Write-Host "  - Estimated: approx 5-10% increase on B1 tier" -ForegroundColor White
...
Write-Host "`n[SUCCESS] Configuration complete!`n" -ForegroundColor Green
